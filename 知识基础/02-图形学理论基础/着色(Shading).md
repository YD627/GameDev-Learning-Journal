...
---
#### 3. 纹理应用 (Texture Application)
1. Environment Map: 可以用纹理贴图去模拟环境光，实现更真实的光照效果。
2. Spherical Environment Map: 将环境光直接记录在球面上
    - 问题: 纹理展开会有扭曲
3. Cube Map: 将环境光记录在立方体上，每个面对应一个纹理贴图。
4. 纹理可以影响渲染
    - Bump/Normal Map(凹凸贴图/法线贴图): 增加表面信息但不需要添加更多的三角形
        - Bump Map: 定义任何一个点相对的高度移动，通过高度来计算法线
        - 先求切线(微分方法), 然后切线逆时针旋转90度, 就是法线
        - 3D情况下: n = (-dp/du, dp/dv, 1).normalize()
    - Displacement Map: 通过高度来移动顶点位置，实现更复杂的表面效果
    - 3D Procedural Noise +  Solid Modeling: 生成复杂的模型，如山脉、森林等

---
#### 4. Shadow Mapping(阴影映射)
1. Render from light: 从光源看向场景，记录能看到的场景物体的深度值
2. Render from eye: 从观察方向看场景，将看到的点到光源的距离与之前的深度值进行比较
    - 如果看到的点到光源的距离大于之前的深度值, 则该点在阴影中
    - 否则, 则该点不在阴影中