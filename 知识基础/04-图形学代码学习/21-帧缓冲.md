...
glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 800, 600, GL_RGB, GL_UNSIGNED_BYTE, NULL);
...
```C++
glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24_STENCIL8, 800, 600);
```
...
void main()
{
    ...
    for(int i = 0; i < 9; i++)
    {
        sampleTex[i] = texture(screenTexture, TexCoords + offset[i]);
    }
